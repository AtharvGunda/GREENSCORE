 ...
 */
const calculateEmissions = (record, revenue_cr) => {
    ...
    const scope1_elec = (electricity_kwh * emissionFactors_js_1.EMISSION_FACTORS.scope1.electricity_grid_kwh) / 1000; // in Tonnes
    const diesel_l = (record.diesel_litres_per_cr || 0) * revenue_cr;
    const scope1_diesel = (diesel_l * emissionFactors_js_1.EMISSION_FACTORS.scope1.diesel_litre) / 1000;
    const lpg_kg = (record.lpg_kg_per_cr || 0) * revenue_cr;
    const scope1_lpg = (lpg_kg * emissionFactors_js_1.EMISSION_FACTORS.scope1.lpg_kg) / 1000;
    const petrol_l = (record.petrol_litres_per_cr || 0) * revenue_cr;
    const scope1_petrol = (petrol_l * emissionFactors_js_1.EMISSION_FACTORS.scope1.petrol_litre) / 1000;
    ...
    const scope2_steam = (steam_gj * emissionFactors_js_1.EMISSION_FACTORS.scope2.purchased_steam_gj) / 1000;
    const total_co2e_scope2 = scope2_steam;
    // Calculate Scope 3
    const travel_km = (record.business_travel_km_per_cr || 0) * revenue_cr;
    const scope3_travel = (travel_km * emissionFactors_js_1.EMISSION_FACTORS.scope3.road_travel_km_per_person) / 1000;
